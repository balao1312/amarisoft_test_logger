...
class Iperf3_logger(Amari_logger):
    ...
    def __init__(self, port, notify_when_terminated, label, dont_send_to_db, project_field_name, notify_dst):
        super().__init__()
        self.port = port
        ...
        if self.notify_when_terminated:
    ...
    def check_if_client_is_terminated_and_send_notify_on_demand(self, line):
        if self.terminated_pattern.match(line) and self.notify_when_terminated:
            msg = f'{datetime.now().strftime("%Y-%m-%d %H:%M:%S")}\nAs iperf sever, the client side has terminated session.'
    ...
    def check_if_is_in_parallel_mode(self, line):
        if 'connected to' in line:
            self.num_incoming_threads += 1
        return
    ...
    def run_iperf3_session(self):
        ...
        while True:
            ...
            # count the incoming threads to determine parallel mode
            self.check_if_is_in_parallel_mode(line)
            ...
            self.check_if_client_is_terminated_and_send_notify_on_demand(line)

            # get throughput according to numbers of incoming thread
            if self.num_incoming_threads < 2:
